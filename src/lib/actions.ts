...
import type { Product, LineItem, OrderStatus, SiteContent, ExploreSection, InSituSection, ImageAsset, AboutPageContent, TeamMember, ProductPageContent, Guarantee, Testimonial, Notification } from './types';
...
export async function createOrder(formData: FormData, items: LineItem[], total: number) {
  ...
  try {
    ...
    const notification: Notification = {
      id: uuidv4(),
      type: 'new_order',
      message: `New order #${newOrder.id.substring(0, 4)} from ${newOrder.customerName}.`,
      href: `/admin/dashboard/orders/${newOrder.id}`,
      isRead: false,
      createdAt: serverTimestamp(),
    };
...
export async function markNotificationAsRead(notificationId: string) {
    ...
    try {
        ...
        revalidatePath('/admin/dashboard'); // Revalidate to update the UI
    } catch (error) {
...
export async function updateAllLogos(formData: FormData) {
  ...
  try {
    ...
    const logoFields: (keyof SiteContent)[] = ['headerLogoLight', 'headerLogoDark', 'footerLogoLight', 'footerLogoDark'];
...
export async function updateAdminLogos(formData: FormData) {
  const firestore = getDb();
  const contentRef = doc(firestore, 'site_content', 'homepage');
  ...
  try {
    ...
    const logoFields: (keyof SiteContent)[] = ['adminLogoLight', 'adminLogoDark', 'adminLogoShort'];
    ...
    revalidatePath('/admin/dashboard/settings/admin');
    revalidatePath('/admin', 'layout');

  } catch (error) {
    console.error('Error updating admin logos:', error);
    throw new Error('Failed to update admin logos.');
  }
}
