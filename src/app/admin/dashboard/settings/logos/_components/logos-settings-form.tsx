...
const LogoUploader = ({ name, label, content, currentLogo, preview, bgClass = 'bg-white' }: any) => {
  return (
    <div className="space-y-2">
      <h3 className="font-medium text-sm">{label}</h3>
       <div className={`p-4 border rounded-lg h-24 flex items-center justify-center ${bgClass}`}>
          {preview ? (
            <Image src={preview} alt={`${label} preview`} width={150} height={40} className="object-contain" />
          ) : currentLogo ? (
            <Image
              src={currentLogo.url}
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} encType="multipart/form-data">
          <CardHeader>
            ...
            <CardDescription>Update your site's logos for the header, footer, and admin panel.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-8">
            ...
            <div>
             ...
             <div>
              <h3 className="text-lg font-semibold mb-4 border-b pb-2">Admin Panel Logos</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start pt-4">
                <LogoUploader name="adminLogoLight" label="Light Logo (for dark backgrounds)" content={form.control} currentLogo={content.adminLogoLight} preview={previews.adminLogoLight} bgClass="bg-gray-800" />
                <LogoUploader name="adminLogoDark" label="Dark Logo (for light backgrounds)" content={form.control} currentLogo={content.adminLogoDark} preview={previews.adminLogoDark} bgClass="bg-white" />
              </div>
            </div>
          </CardContent>
