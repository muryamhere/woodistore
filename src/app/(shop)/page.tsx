  ...
  return (
    <div className="md:px-0">
      ...
      <section className="w-full py-12 md:py-24">
        <div className="relative aspect-video w-full h-full overflow-hidden rounded-2xl container mx-auto">
            <Image
                    ...
                    <div className="absolute top-[25%] left-[35%] w-1/4 max-w-xs transform -translate-x-1/2 -translate-y-1/2">
                        <Link href={`/products/${content.inSituSection.spots[0].productId}`} className="block group">
                            <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-2xl p-2 md:p-3 group-hover:scale-105 transition-transform duration-300">
                                ...
                                <h4 className="font-semibold text-xs md:text-sm">{inSituProducts[0]?.name ?? content.inSituSection.spots[0].title}</h4>
                                <p className="text-xs md:text-sm font-medium text-foreground/80">Rs. {inSituProducts[0]?.price.toFixed(2) ?? content.inSituSection.spots[0].price}</p>
                                </div>
                            </div>
                        </Link>
                    </div>
                 )}
                 {content.inSituSection?.spots[1] && (
                    <div className="absolute bottom-[25%] right-[35%] w-1/4 max-w-xs transform translate-x-1/2 translate-y-1/2">
                       <Link href={`/products/${content.inSituSection.spots[1].productId}`} className="block group">
                            <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-2xl p-2 md:p-3 group-hover:scale-105 transition-transform duration-300">
                                ...
                                <div className="flex justify-between items-center mt-2 md:mt-3">
                                <h4 className="font-semibold text-xs md:text-sm">{inSituProducts[1]?.name ?? content.inSituSection.spots[1].title}</h4>
                                <p className="text-xs md:text-sm font-medium text-foreground/80">Rs. {inSituProducts[1]?.price.toFixed(2) ?? content.inSituSection.spots[1].price}</p>
                                </div>
                            </div>
                        </Link>
                    </div>
